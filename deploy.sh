...
deploy_cluster() {
    ...
    make_task_def
    register_definition
    if [[ $(aws ecs update-service --cluster default --service archimedes --task-definition $revision | \
    ...
    for attempt in {1..30}; do
        if stale=$(aws ecs describe-services --cluster default --services archimedes | \
